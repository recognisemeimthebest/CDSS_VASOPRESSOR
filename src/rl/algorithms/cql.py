"""Conservative Q-Learning — Kumar et al. 2020 (discrete variant, CQL(H)).

Adds a conservative penalty that pushes down Q at all actions except the
in-batch one:

    L_cql = alpha * (logsumexp_a Q(s, a) - Q(s, a_data))

This keeps OOD Q values tight without needing behaviour-policy estimation.
"""
...
class ConservativeQLearning(BaseAgent):
    def __init__(
        self,
        in_features: int,
        n_actions: int,
    ...
    def update(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        td_loss = torch.nn.functional.smooth_l1_loss(q_taken, target)
